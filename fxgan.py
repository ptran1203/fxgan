...
class SelfAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        # Set input spec.
        self.input_spec = keras.layers.InputSpec(ndim=4, axes={3: input_shape[-1]})
        self.built = True

    def call(self, x):
        def hw_flatten(x):
            return K.reshape(
                x, shape=[K.shape(x)[0], K.shape(x)[1] * K.shape(x)[2], K.shape(x)[3]]
            )
        ...
        s = K.batch_dot(
            hw_flatten(g), K.permute_dimensions(hw_flatten(f), (0, 2, 1))
        )  # # [bs, N, N]
...
class FXGAN:
    ...
    def _dc_block(self, x, units, kernel_size=3, activation="relu", norm="batch"):
    ...
    def show_samples_for_class(self, bg, classid, mode="00"):
        ...
        if mode_x == 0:
            ...
            support_images = bg.ramdom_kshot_images(self.k_shot, [classid] * samples)
        else:
            print("same images")
            support_images = bg.ramdom_kshot_images(self.k_shot, [classid])
            support_images = np.repeat(support_images, samples, axis=0)
        if mode_y == 0:
    ...
    def build_latent_encoder(self):
        fname = "{}/{}/latent_encoder_{}".format(
            BASE_DIR, self.dataset, self.resolution
        )
        if self.env != "colab":
    ...
    def gen_for_class(self, bg, bg_test=None, classid=0, size=1000):
        ...
        for i in range(1000):
            ...
            if classid in bg.classes:
                support = bg.ramdom_kshot_images(self.k_shot, np.full(size, classid))
            else:
                if bg_test is None:
                    raise ("bg_test is None, please give it, boi")

                support = bg_test.ramdom_kshot_images(
                    self.k_shot, np.full(size, classid)
                )

            gen = self.generate(support, latent)
            d_outputs = self.classify_by_metric(
                bg, gen, bg_test=bg_test, anchor=(support, classid)
            )
    ...
    def latent_codes(self, k_shot_images, prediction=True):
        ...
        """
        return np.array([np.mean(self.latent_code(i), axis=0) for i in k_shot_images])
    ...
    def __init__(
        ...
        if self.loss_type != "categorical":
            ...
            self.discriminator_fake = Model(
                inputs=[fake_images], outputs=fake_output_for_d, name="D_fake",
            )
            ...
            self.discriminator_real = Model(
                inputs=[real_images], outputs=real_output_for_d, name="D_real",
            )
        ...
        fake = self.generator([real_images_for_G, attr_features, latent_code])
    ...
    def encode_image(self, image):
        ...
        i = 0
        while width != 4:
            x_temp = Conv2D(channels, kernel_size, strides=2, padding="same")(
                connections[i]
            )
    ...
    def build_dc_generator(self):
        ...
        final = self._dc_block(
            de, self.channels, kernel_size, activation="tanh", norm=None
        )

        self.generator = Model(
            inputs=[image, attribute_code, latent_code], outputs=final, name="dc_gen"
        )
    ...
    def build_discriminator(self):
        ...
        aux = Dense(last_channels, activation=activation, name="auxiliary")(features)
    ...
    def generate_latent(self, c, size=1):
        ...
        return np.array([np.random.normal(0, 1, self.latent_size) for i in c])
    ...
    def _train_one_epoch(self, bg_train):
        ...
        for image_batch, label_batch in bg_train.next_batch():
            ...
            for i in range(self.D_RATE):
                ...
                if self.loss_type == "categorical":
                    ...
                    loss, acc = self.discriminator.train_on_batch(
                        ...
                        np.concatenate(
                            [real_label, np.full(crt_batch_size, self.nclasses)], axis=0
                        ),
                        class_weight=class_weight,
                    )
                else:
                    loss_fake, acc_fake, *rest = self.discriminator_fake.train_on_batch(
                        [generated_images], fake_label
                    )
                    loss_real, acc_real, *rest = self.discriminator_real.train_on_batch(
                        [image_batch], real_label
                    )
            ...
            ################## Train Generator ##################
            f = self.generate_latent(label_batch)
            negative_samples = bg_train.get_samples_by_labels(
                bg_train.other_labels(label_batch)
            )
    ...
    def _get_lst_bck_name(self, element):
        # Find last bck name
        files = [f for f in os.listdir(self.res_dir) if re.match(r"bck_" + element, f)]
        if len(files) > 0:
            fname = files[0]
            epoch = 0
            return epoch, fname
    ...
    def plot_cm_for_G(self, bg, bg_test=None, labels=None, metric="l2"):
        ...
        support_images = bg.ramdom_kshot_images(self.k_shot, labels[train_mask])
        if bg_test is not None:
            support_images = np.concatenate(
                [
                    support_images,
                    bg_test.ramdom_kshot_images(self.k_shot, labels[test_mask]),
                ]
            )
        ...
        plt.figure()
        plot_confusion_matrix(cm, hide_ticks=False, cmap=plt.cm.Blues, figsize=(8, 8))
        plt.show()
    ...
    def train(self, bg_train, bg_test, epochs=50):
        if not self.trained:
            ...
            act_img_samples = bg_train.ramdom_kshot_images(
                self.k_shot, np.full(10, crt_c)
            )
            ...
            for e in range(start_e, epochs):
                ...
                k_shot_test_batch = bg_train.ramdom_kshot_images(
                    self.k_shot, test_batch_y
                )
                ...
                negative_samples = bg_train.get_samples_by_labels(
                    bg_train.other_labels(test_batch_y)
                )
                ...
                if e % (50 // (self.resolution // 32)) == 0:
                    self.backup_point(e)
                    self.evaluate_d(
                        np.concatenate(X, axis=0), np.concatenate(Y, axis=0)
                    )
                    self.evaluate_g(
                        [k_shot_test_batch, f,], real_label,
                    )
                    ...
                    self.plot_cm_for_G(bg_train, bg_test, classes)
    ...
    def plot_feature_distr(self, bg, size=50):
        ...
        real = bg.ramdom_kshot_images(self.k_shot, np.full(size, bg.classes[0]))
        ...
        for classid in bg.classes[1:5]:
            real = bg.ramdom_kshot_images(self.k_shot, np.full(size, classid))
        ...
        labels = np.concatenate(
            [np.full((x.shape[0],), "real"), np.full((fakes.shape[0],), "fake"),]
        )

        try:
            utils.scatter_plot(
                imgs, labels, self.features_from_d_model, "fake real space"
            )
        except:
